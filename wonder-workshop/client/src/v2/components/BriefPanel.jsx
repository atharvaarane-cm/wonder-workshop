...
export function BriefPanel({
  ...
  useEffect(() => {
    if (editing) autoSize(); /* eslint-disable-next-line */
  }, [draft]);
  ...
  return (
    <div
      style={{
        borderTop: "1px solid var(--warm-06)",
        margin: "20px 0",
        padding: "20px 0 0",
      }}
    >
      ...
      <Card
        ...
        className={[
          ...
          editing
            ? "min-h-[120px] max-h-[600px] cursor-default ring-1 ring-ring/40"
            : "min-h-24 max-h-24 cursor-pointer",
        ].join(" ")}
        style={{
          WebkitMaskImage: editing
            ? "none"
          ...
          >
            {treatmentText || (
              <span style={{ opacity: 0.3 }}>
                Click here to write or paste the brief — the spot's setup,
                characters, tone, and intent.
              </span>
            )}
          </div>
        )}
      ...
      {editing && (
        ...
        >
          ...
          <button
            ...
            disabled={!dirty}
            title={
              dirty ? "Save brief and audit changes (⌘↵)" : "No changes to save"
            }
            style={{
              fontFamily: "var(--f)",
              fontSize: 12,
      ...
      <AlertDialog open={open} onOpenChange={setOpen}>
        <AlertDialogTrigger
          render={
            <Button
              variant="outline"
              size="sm"
              className="gap-1.5 dark:border-white/18 dark:bg-[#181818] dark:shadow-[0_1px_2px_rgba(0,0,0,0.42),inset_0_1px_0_rgba(255,255,255,0.045)]"
            />
          }
        ...
        <AlertDialogPopup className="max-w-2xl dark:border-white/18 dark:bg-[#181818] dark:shadow-[0_20px_60px_rgba(0,0,0,0.65),inset_0_1px_0_rgba(255,255,255,0.045)]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Update the project brief. Save turns on after you make a change,
              then runs the brief-change audit.
            </AlertDialogDescription>
          </AlertDialogHeader>
...
export function BriefSettingsCard({
  value,
  onUpdateMeta,
  data,
  onRunRegeneration,
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
            <Button variant="outline" size="xs" onClick={handleCancel} disabled={!dirty}>
              Cancel
            </Button>
            <Button
              variant="outline"
...
function RegenerateAuditModal({ oldBrief, newBrief, data, onClose, onRun }) {
  ...
  const renderCategoryLines = (label, cat, collection) => {
    if (!cat) return null;
    const lines = [];
    (cat.update || []).forEach((id) =>
      lines.push(`Update ${label}: ${idToName(collection, id)}`),
    );
    (cat.add || []).forEach((desc) => lines.push(`Add ${label}: ${desc}`));
    (cat.remove || []).forEach((id) =>
      lines.push(`Remove ${label}: ${idToName(collection, id)}`),
    );
    return lines;
  };
        ...
        {phase === "error" && (
          ...
          >
            ...
            <span style={{ color: "rgba(255,255,255,0.55)" }}>
              You can still regenerate everything or cancel.
            </span>
          </div>
        )}
        ...
        {phase === "ready" && (
          <>
            ...
            {allLines.length === 0 ? (
              ...
              >
                No targeted changes detected. You can still regenerate
                everything.
              </div>
            ) : (
              <ul
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              background:
                phase === "loading" ? "rgba(255,255,255,0.08)" : "#fff",
              border:
                "1px solid " +
                (phase === "loading" ? "var(--warm-12)" : "#fff"),
